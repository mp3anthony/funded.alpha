...
-- 9. Drop and recreate frequency check constraints to permit 'fortnightly' and 'fortnightly'
ALTER TABLE pay_schedules DROP CONSTRAINT IF EXISTS pay_schedules_frequency_check;
ALTER TABLE pay_schedules ADD CONSTRAINT pay_schedules_frequency_check CHECK (frequency IN ('weekly', 'fortnightly', 'fortnightly', 'monthly', 'yearly'));

ALTER TABLE bills DROP CONSTRAINT IF EXISTS bills_frequency_check;
ALTER TABLE bills ADD CONSTRAINT bills_frequency_check CHECK (frequency IN ('weekly', 'fortnightly', 'fortnightly', 'monthly', 'yearly'));

ALTER TABLE household_contributions DROP CONSTRAINT IF EXISTS household_contributions_frequency_check;
ALTER TABLE household_contributions ADD CONSTRAINT household_contributions_frequency_check CHECK (frequency IN ('weekly', 'fortnightly', 'fortnightly', 'monthly', 'yearly'));
